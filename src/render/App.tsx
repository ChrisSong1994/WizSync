...
function App() {
  ...
  const [currentTask, setCurrentTask] = useState<Partial<SyncTask> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    window.electronAPI.onSyncStatus(({ id, status, lastSyncTime }) => {
      setTasks((prev) =>
        prev.map((t) => (t.id === id ? { ...t, status, lastSyncTime } : t)),
      );
    });
  ...
  const handleSaveTask = async () => {
    if (
      ...
      const updatedTasks = await window.electronAPI.saveTask(
        currentTask as SyncTask,
      );
      setTasks(updatedTasks);
            ...
            <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-xl font-bold text-slate-800">
                  同步日志 - {tasks.find((t) => t.id === selectedTaskId)?.name}
                </h2>
              </div>
              <button
